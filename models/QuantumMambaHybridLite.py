...
"""
Quantum Mamba Hybrid Lite - Hydra-Matched Architecture

This is a lightweight variant of Quantum Mamba Hybrid that matches the Quantum Hydra
Hybrid architecture exactly, using timestep loop processing with learnable real-valued
temporal weights (with softmax) instead of Conv1d.

Key Architectural Changes from QuantumMambaHybridTS:
1. REMOVED: Conv1d temporal processing (12,352 params)
2. REMOVED: AdaptiveAvgPool1d
3. ADDED: Timestep loop processing (matching Quantum Hydra Hybrid)
...
- QuantumMambaHybridTS_Lite:      ~7,196 parameters (matches Quantum Hydra Hybrid)
...
class QuantumMambaHybridTS_Lite(nn.Module):
    """
    Lightweight Quantum Mamba Hybrid for Time-Series matching Quantum Hydra Hybrid architecture.

    Processes sequential data using Option B (classical combination) with timestep loop
    processing, identical to Quantum Hydra Hybrid's approach.
    ...
    def forward(self, x):
        ...
        # Process each timestep independently (matching Quantum Hydra Hybrid)
        timestep_outputs = []
        for t in range(self.n_timesteps):
            x_t = x[:, t, :]  # (batch_size, feature_dim)
            x_t_proj = self.feature_projection(self.dropout(x_t))
            out_t = self.quantum_mamba(x_t_proj)  # (batch_size, 3*n_qubits)
        ...
        # (matching Quantum Hydra Hybrid's real-valued weighted aggregation)
