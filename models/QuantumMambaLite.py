...
"""
Quantum Mamba Lite (Superposition) - Hydra-Matched Architecture

This is a lightweight variant of Quantum Mamba that matches the Quantum Hydra
architecture exactly, using timestep loop processing with learnable complex-valued
temporal weights instead of Conv1d.

Key Architectural Changes from QuantumMambaTS:
...
- QuantumMambaTS_Lite:         ~7,196 parameters (matches Quantum Hydra)
...
class QuantumMambaTS_Lite(nn.Module):
    """
    Lightweight Quantum Mamba for Time-Series matching Quantum Hydra architecture.

    Processes sequential data by treating each timestep with Quantum Mamba,
    identical to Quantum Hydra's timestep loop processing.
    ...
    def forward(self, x):
        ...
        # Process each timestep independently (matching Quantum Hydra)
        timestep_outputs = []
        ...
        # (matching Quantum Hydra's quantum superposition aggregation)
