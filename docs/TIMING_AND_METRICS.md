...
**YES**, all experiment scripts in both `/pscratch/sd/j/junghoon/experiments/` and `/pscratch/sd/j/junghoon/quantum_hydra_mamba_repo/experiments/` **DO record timing information** for each experiment.
...
**Both experiment folders record complete timing information:**

### `/pscratch/sd/j/junghoon/experiments/`
✅ Records `training_time` in results JSON
✅ Records per-epoch time during training
✅ Records timestamp for each run

### `/pscratch/sd/j/junghoon/quantum_hydra_mamba_repo/experiments/`
✅ Same timing recording as original folder
✅ All 4 experiment scripts track timing
...
- Each dataset (EEG, MNIST, DNA, Forrelation)
- Each experiment configuration

**File format:** JSON files saved in `{dataset}_results/` directories
