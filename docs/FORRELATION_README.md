...
```
experiments/
├── run_single_model_forrelation.py       # Main training script
├── generate_forrelation_datasets_all.sh  # Generate all datasets
├── generate_forrelation_job_scripts.py   # Generate SLURM scripts
├── submit_forrelation_experiments.sh     # Submit all jobs
├── aggregate_forrelation_results.py      # Analyze results
├── forrelation_job_scripts/              # Generated SLURM scripts
└── forrelation_results/                  # Outputs (JSON, models, plots)
    └── logs/                             # SLURM logs

forrelation_data/                         # Datasets
...
```bash
bash experiments/generate_forrelation_datasets_all.sh
```
...
```bash
python experiments/generate_forrelation_job_scripts.py
```
...
```bash
bash experiments/submit_forrelation_experiments.sh
```
...
```bash
python experiments/run_single_model_forrelation.py --batch-size 16
```
...
For questions or issues, refer to the main project documentation or check the logs in `experiments/forrelation_results/logs/`.
